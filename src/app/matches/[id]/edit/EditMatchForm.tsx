...
import { Member, Match } from "@/lib/types";
...
export default function EditMatchForm({
  ...
  const [team1p2, setTeam1p2] = useState(match.team1[1] ?? "");
  const [team2p1, setTeam2p1] = useState(match.team2[0] ?? "");
  const [team2p2, setTeam2p2] = useState(match.team2[1] ?? "");
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-green-800/40">
        ...
        <div className="flex gap-3">
          <SelectField label="プレイヤー1（あなた）" value={MY_NAME} onChange={() => {}} disabled />
          <SelectField
            label="パートナー（任意）"
            value={team1p2}
            onChange={setTeam1p2}
